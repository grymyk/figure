...
Shape.Shoulder.prototype = {
    setOptions : function(shape) {
        this.options = this.shapeType[shape];
    },
    
    drawShape : function(curve) {
        var LEFT_SHOULDER_BOTTOM_CP_X = this.options.LEFT_SHOULDER_BOTTOM_CP_X;
        var DELTA_ORIGIN_X = this.options.DELTA_ORIGIN_X;
        var DELTA_X_TOP = 0.1;
        var DELTA_X_BOTTOM = this.options.DELTA_X_BOTTOM;
        var DELTA_Y_TOP = 0.3;
        var DELTA_Y_BOTTOM = 0.3;
        
        var SHIFT_X_ORIGIN = this.size.width * 0.08;
    ...
    eraseShape : function(curve) {
        var xShoulder = this.origin.x - this.size.width * 0.65;
        var yShoulder = this.origin.y;
        var widthShoulder = this.size.width * 1.3;
        var heightShoulder = this.size.height;
